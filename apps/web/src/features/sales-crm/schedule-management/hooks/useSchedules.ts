...
export function useSyncToGoogleCalendar() {
  ...
  return useMutation({
    mutationFn: (id: string) => scheduleService.syncToGoogleCalendar(id),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: ["schedules"] });
      queryClient.invalidateQueries({ queryKey: ["schedules", id] });
      toast.success("Schedule synced to Google Calendar successfully");
    },
    onError: (error: unknown) => {
...
export function useUnsyncFromGoogleCalendar() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => scheduleService.unsyncFromGoogleCalendar(id),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: ["schedules"] });
      queryClient.invalidateQueries({ queryKey: ["schedules", id] });
      toast.success("Schedule unsynced from Google Calendar successfully");
    },
    onError: (error: unknown) => {
      const message =
        error instanceof Error
          ? error.message
          : "Failed to unsync schedule from Google Calendar";
      toast.error(message);
    },
  });
}
