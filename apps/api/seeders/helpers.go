...
func buildCRMSeedRecords(ctx *seedFixtureContext) []crmSeedRecord {
	...
	scenarios := []scenario{
		...
		{
			key:                 "deal-won-2",
			leadEmail:           "nisa.prameswari@puskesmassehat.id",
			firstName:           "Nisa",
			lastName:            "Prameswari",
			companyName:         "Puskesmas Sehat Abadi",
			phone:               "081234567894",
			jobTitle:            "Head Procurement",
			leadSource:          "referral",
			leadStatusCode:      "qualified",
			leadNotes:           "Lead siap closing untuk paket kebutuhan operasional dan produk rawat jalan.",
			address:             "Jl. Pandanaran No. 24",
			city:                "Semarang",
			province:            "Jawa Tengah",
			postalCode:          "50129",
			website:             "https://puskesmassehat.id",
			industry:            "Healthcare",
			budgetAmount:        760000000,
			authorityPerson:     "Nisa Prameswari",
			needDescription:     "Butuh paket obat umum, vitamin, dan alat kesehatan untuk kebutuhan bulanan.",
			needPriorityLevel:   "high",
			estimatedValue:      820000000,
			leadProbability:     88,
			dealStageCode:       "closed_won",
			dealTitle:           "Closed Won - Puskesmas Sehat Abadi Monthly Supply",
			dealDescription:     "Kontrak supply bulanan sudah disetujui dan masuk tahap fulfilment.",
			dealCloseReason:     "Kontrak bulanan disetujui oleh pengelola puskesmas.",
			visitPurpose:        "Konfirmasi final kebutuhan supply bulanan",
			visitNotes:          "Poin kebutuhan dan jadwal kirim sudah disepakati.",
			visitOutcome:        "very_positive",
			visitNextSteps:      "Koordinasi detail pengiriman dan dokumentasi kontrak.",
			visitStatus:         "approved",
			visitOffsetDays:     -4,
			visitDurationHours:  1,
			activityTypeCode:    "document_proposal_sent",
			activityDescription: "Kontrak dan lampiran produk sudah dikirim ke puskesmas.",
			taskTitle:           "Follow up fulfilment Puskesmas Sehat Abadi",
			taskDescription:     "Pastikan jadwal kirim dan dokumen kontrak diproses tepat waktu.",
			taskType:            "follow_up",
			taskPriority:        "high",
			taskSource:          "pipeline_tab",
			productIndices:      []int{1, 4},
			interestLevels:      []int{5, 4},
			quantities:          []int{18, 26},
		},
		{
			key:                 "deal-won-3",
			leadEmail:           "farhan.maulana@klinikutama.id",
			firstName:           "Farhan",
			lastName:            "Maulana",
			companyName:         "Klinik Utama Harmoni",
			phone:               "081234567895",
			jobTitle:            "Clinic Director",
			leadSource:          "event",
			leadStatusCode:      "qualified",
			leadNotes:           "Lead closing dengan kebutuhan paket obat resep dan alat kesehatan rawat jalan.",
			address:             "Jl. Diponegoro No. 51",
			city:                "Semarang",
			province:            "Jawa Tengah",
			postalCode:          "50130",
			website:             "https://klinikutama.id",
			industry:            "Clinic",
			budgetAmount:        690000000,
			authorityPerson:     "Farhan Maulana",
			needDescription:     "Membutuhkan paket rawat jalan untuk ekspansi layanan klinik.",
			needPriorityLevel:   "critical",
			estimatedValue:      735000000,
			leadProbability:     92,
			dealStageCode:       "closed_won",
			dealTitle:           "Closed Won - Klinik Utama Harmoni Expansion Package",
			dealDescription:     "Paket ekspansi layanan klinik disetujui dan siap dijalankan.",
			dealCloseReason:     "Customer menyetujui paket ekspansi dan PO diterbitkan.",
			visitPurpose:        "Final closing untuk paket ekspansi klinik",
			visitNotes:          "Final review harga dan timeline pengiriman selesai.",
			visitOutcome:        "very_positive",
			visitNextSteps:      "Koordinasi jadwal implementasi dan onboarding produk.",
			visitStatus:         "approved",
			visitOffsetDays:     -1,
			visitDurationHours:  1,
			activityTypeCode:    "presentation_demo_meet",
			activityDescription: "Presentasi akhir dan demo penggunaan produk untuk tim klinik.",
			taskTitle:           "Koordinasi implementasi Klinik Utama Harmoni",
			taskDescription:     "Lanjutkan koordinasi implementasi setelah deal berhasil ditutup.",
			taskType:            "meeting",
			taskPriority:        "urgent",
			taskSource:          "pipeline_tab",
			productIndices:      []int{0, 2, 4},
			interestLevels:      []int{5, 4, 5},
			quantities:          []int{16, 12, 20},
		},
		{
			key:                 "deal-won-4",
			leadEmail:           "salsa.novita@rsbhaktisehat.id",
			firstName:           "Salsa",
			lastName:            "Novita",
			companyName:         "RS Bhakti Sehat",
			phone:               "081234567896",
			jobTitle:            "Procurement Lead",
			leadSource:          "website",
			leadStatusCode:      "qualified",
			leadNotes:           "Lead siap final approval untuk paket alat kesehatan dan obat rawat inap.",
			address:             "Jl. Imam Bonjol No. 77",
			city:                "Semarang",
			province:            "Jawa Tengah",
			postalCode:          "50131",
			website:             "https://rsbhaktisehat.id",
			industry:            "Hospital",
			budgetAmount:        1280000000,
			authorityPerson:     "Salsa Novita",
			needDescription:     "Membutuhkan paket alat kesehatan rawat inap dan stock penunjang ruang perawatan.",
			needPriorityLevel:   "high",
			estimatedValue:      1345000000,
			leadProbability:     94,
			dealStageCode:       "closed_won",
			dealTitle:           "Closed Won - RS Bhakti Sehat Inpatient Package",
			dealDescription:     "Pengadaan paket rawat inap dan alat kesehatan sudah disetujui.",
			dealCloseReason:     "Komite pengadaan menyetujui paket final.",
			visitPurpose:        "Final review pengadaan rawat inap",
			visitNotes:          "Detail item dan jadwal implementasi sudah dikunci.",
			visitOutcome:        "very_positive",
			visitNextSteps:      "Koordinasi pengiriman dan persiapan implementasi gudang.",
			visitStatus:         "approved",
			visitOffsetDays:     -2,
			visitDurationHours:  1,
			activityTypeCode:    "document_proposal_sent",
			activityDescription: "Dokumen final pengadaan rawat inap telah dikirim.",
			taskTitle:           "Follow up implementasi RS Bhakti Sehat",
			taskDescription:     "Pastikan pengiriman dan serah terima dokumen berjalan lancar.",
			taskType:            "follow_up",
			taskPriority:        "high",
			taskSource:          "pipeline_tab",
			productIndices:      []int{0, 1, 3},
			interestLevels:      []int{5, 4, 5},
			quantities:          []int{28, 24, 18},
		},
		{
			key:                 "deal-won-5",
			leadEmail:           "rafi.prasetyo@apoteknusa.id",
			firstName:           "Rafi",
			lastName:            "Prasetyo",
			companyName:         "Apotek Nusa Farma",
			phone:               "081234567897",
			jobTitle:            "Owner",
			leadSource:          "referral",
			leadStatusCode:      "qualified",
			leadNotes:           "Lead konversi untuk pembelian rutin dan penguatan stok produk OTC.",
			address:             "Jl. MT Haryono No. 88",
			city:                "Semarang",
			province:            "Jawa Tengah",
			postalCode:          "50132",
			website:             "https://apoteknusafarma.id",
			industry:            "Pharmacy",
			budgetAmount:        540000000,
			authorityPerson:     "Rafi Prasetyo",
			needDescription:     "Butuh replenishment produk OTC, vitamin, dan alat kesehatan ringan.",
			needPriorityLevel:   "medium",
			estimatedValue:      585000000,
			leadProbability:     89,
			dealStageCode:       "closed_won",
			dealTitle:           "Closed Won - Apotek Nusa Farma Replenishment",
			dealDescription:     "Order replenishment rutin telah disetujui dan masuk antrian kirim.",
			dealCloseReason:     "Kesepakatan pembelian berulang sudah final.",
			visitPurpose:        "Finalisasi replenishment dan jadwal kirim",
			visitNotes:          "Permintaan stok dan timeline kirim sudah final.",
			visitOutcome:        "very_positive",
			visitNextSteps:      "Koordinasi delivery dan data PO final.",
			visitStatus:         "approved",
			visitOffsetDays:     -1,
			visitDurationHours:  1,
			activityTypeCode:    "presentation_demo_meet",
			activityDescription: "Review final order dan konfirmasi kebutuhan replenishment.",
			taskTitle:           "Koordinasi delivery Apotek Nusa Farma",
			taskDescription:     "Pastikan pengiriman replenishment sesuai PO final.",
			taskType:            "follow_up",
			taskPriority:        "high",
			taskSource:          "pipeline_tab",
			productIndices:      []int{2, 4},
			interestLevels:      []int{4, 5},
			quantities:          []int{30, 12},
		},
		{
			key:                 "deal-lost",
