...
func composeAIContext(plan aiQueryPlan, retrieved aiRetrievedContext) string {
	...
	sb.WriteString("- Jika data kosong, katakan tidak ada record yang cocok untuk periode/filter/scope tersebut. Jangan menyebut tidak punya akses kecuali backend secara eksplisit memberi pesan permission/privacy denied.\n")
	sb.WriteString("- Jangan menyimpulkan batas periode data seperti `hanya sampai Juli` kecuali tanggal tersebut tertulis di VERIFIED DATA.\n")
